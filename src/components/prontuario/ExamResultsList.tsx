    ...
    return (
      <Card className="p-4 md:p-6 border-primary/30 hover:scale-[1.02] transition-transform duration-200">
        <div className="flex items-center gap-2 md:gap-3 mb-3 md:mb-4">
          ...
          <h2 className="text-lg sm:text-xl font-semibold text-foreground">Exames Recentes</h2>
        </div>
          ...
          return (
            <div key={record.id} className="p-3 bg-primary-light/20 rounded-lg border border-primary/10">
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1 sm:gap-2 mb-1">
                <p className="text-sm sm:text-base font-medium text-foreground break-words">{record.title}</p>
                <Badge variant="secondary" className="text-xs w-fit">Registrado</Badge>
              </div>
              <p className="text-xs text-muted-foreground mb-2">{formattedDate}</p>
              <p className="text-xs sm:text-sm text-muted-foreground break-words">{record.simplifiedSummary}</p>
            </div>
          );
        })}
      </div>
