...
const SidebarSection = ({ title, subtitle, items, location, markAllAsCompleted = false, isCollapsed = false }: SidebarSectionProps) => {
  ...
  const calculateLinePositions = () => {
    ...
    for (let i = 0; i < items.length - 1; i++) {
      const currentItem = section.querySelector(`[data-nav-item]:nth-child(${i + 1})`) as HTMLElement;
      const nextItem = section.querySelector(`[data-nav-item]:nth-child(${i + 2})`) as HTMLElement;
      
      if (!currentItem || !nextItem) continue;
      
      const currentCircle = currentItem.querySelector('[data-nav-circle]') as HTMLElement;
      const nextCircle = nextItem.querySelector('[data-nav-circle]') as HTMLElement;
      ...
      const currentCenterY = currentRect.top + currentRect.height / 2 - sectionRect.top;
      const nextCenterY = nextRect.top + nextRect.height / 2 - sectionRect.top;
      const circleCenterX = currentRect.left + currentRect.width / 2 - sectionRect.left;
...
export const Sidebar = () => {
  ...
  const isInProgramGroup = programNavItems.some(item => location.pathname === item.to);
